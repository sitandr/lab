...
1) Вакуумный насос
2) Задвижки — верхняя (**a**) соединена с внешней средой (воздухом), нижняя (**b**) соединяет элементы системы
3) Шланг (обозначаем как изолированный резервуар)
4) Вакуумметр (измеритель давления)

Черные соединительные линии обозначают наличие плотного соединения (переходники) между объектами.
...
В самом начале мы использовали пластмассовые "крышки" для того, чтобы закрывать отверстия в переходниках. Однако в первые несколько измерений оказалось, что давление понижается максимум до $1$ mbar, и очень быстро происходит разгерметизация.
...
$$
Так как откачка происходит в одном месте, то распределение давления должно быть линейным (в зависимости от расстояния до насоса). Тогда для любого поперечного сечения переходника входящий поток должен быть равен выходящему, рассмотрим самый прогнозируемый случай (3 измерение). В этой системе у нас есть **5** соединений, в которых происходит набор воздуха: **2** между трубками, 1 между трубкой и шлангом, 1 на манометре, и на 1 на задвижке. Расстояние до первых 4 сильно меньше расстояния до последнего. Рассмотрим конфигурации 3-его измерения. Давление на конце при 200 секундах (почти установилось равновесие) — почти в 5 раз больше. При этом расстояние больше в 50 раз, ($l = 20\ cm$, $L = 1020\ cm$), значит:
$$
P_{pump} = P_p = \lim\limits_{t \rightarrow \infty} P_3 - (P_4 - P_3) \frac{l}{L} = c(P_3 - (P_4 - P_3))\\ 
0 = \sum_{i=1}^{5} c_1 (P_{atm}- P_i) - с_2 P_p
$$

$P_i \ll P_{atm}$, значит, им можно пренебречь, тогда:
$$
5c_1 P_{atm} = с_2 P_p = c_2 c(P_4 - P_3) \frac{l}{L}
$$
...
c_1 = \frac{c_2 P_p1}{2P_{atm}}  \\
  1.16 · 8 · 10^{-2} = 2 c(P_3 - (P_4 - P_3) \frac{l}{L})  = 5P_{p1} => P_{p1} =  1.856 · 10{-2}
$$

Это интересный результат, но неверный (на самом деле в первом эксперименте давление стремится к значениям около $3·10^{-3}$) — стоит проверить формулу с помощью 4 схемы:
$$
\frac{7c_1 P_{atm}}{c_2} = 6.5 · 10^{-2} = P_p < P_5 = 7 · 10^{-2}
$$
Здесь результат получился гораздо лучше. Значит, этот метод не очень хорошо работает для экспериментов без шланга. Так как от времени у нас ничего не зависит, то дело не может быть в его объёме, а значит, проблема в его креплениях. Они другие и заметно менее приспособлены для работы при очень малом давлении.
В таком случае можно сделать вывод, что коэффициент $c_1$ для шланга должен в большую сторону отличатся от коэффициента $c_1$ для трубки. 
Но, несмотря на это, результат отличается менее чем на порядок от реальности, что при нашей точности является неплохой оценкой.
...
Мы смогли привести качественные оценки происходящего, совпадающие с реальными по порядку величины.