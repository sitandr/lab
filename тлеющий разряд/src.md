---
title: Иследование тлеющего и дугового разряда 
date: \today
...
...
![$ p = 1.9·10^{-1} \mbox{ mBar} $](src.assets/image-20220512215514939.png)|
![$ p = 2·10^{-1} \mbox{ mBar} $](src.assets/image-20220512215536936.png)|
![$ p = 4.4·10^{-1} \mbox{ mBar} $](src.assets/image-20220512215633644.png)|
![$ p = 6.1·10^{-1} \mbox{ mBar} $](src.assets/image-20220512215701355.png)
}
...
{subfigures:
![Магнитное отклонение у анода](src.assets/image-20220512221428750.png)|
![Более четкое изображение отклонения у анода](src.assets/image-20220512221453961.png)
}
...
{subfigures:
![Магниты далеко от катода](src.assets/image-20220523004418042.png)|
![Магниты у катода](src.assets/image-20220523004345768.png)
}
...
![Отклонение на высоких давлениях](src.assets/image-20220523004608346.png)
...
Также при пробое было обнаружено резкое увеличение давления в трубке:



![До пробоя](images_src/1-16545403679941.jpg){width=70%}

![После пробоя](images_src/2-16545403745212.jpg){width=70%}

![После "отмены" пробоя](images_src/3.jpg){width=70%}

Возможно, оно происходит из-за того, что при наличии свечения (после пробоя) в трубке выделяется тепло, из-за чего температура внутри увеличивается, и следом за ней — давление.

Мы решили исследовать безразмерную величину — отношение напряжения пробоя к напряжению поддержания разряда:
...
Что тоже интересно,  в данном эксперименте было обнаружено, что существует  интервал положения ручки генератора, на котором при одинаковых условиях сохраняется пробой, но отличается показания напряжения. При $7 - 13$ мБарах это было особенно заметно: различие до 200 В.  

![Первое положение ручки](images_src/1.jpg){width=70%}

![Второе положение](images_src/2.jpg){width=70%}

Для этого эксперимента (с пробоем) мы брали средние значения, которые мы "замечали" на показаниях вольтметра после пробоя. Возможно, если бы мы брали минимальные или максимальные, разброс бы уменьшился и можно было бы увидеть четкую зависимочть.
