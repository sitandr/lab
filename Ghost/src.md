...
Начнём с того что введём углы: $\alpha $ - это угол в вершине призмы, сопряжённой с 2 пересекаемыми сторонами, $\gamma$ и $\beta$ - углы к нормалям после первого и второго преломления соответственно,  $\theta$ - это суммарный угол отклонения луча от начального направления движения, и $\theta_{red}$ как следствие - этот угол для красного луча.

Но так как такая задача была в дз, то воспользуемся тем, что при минимальном угле отклонения угол падения на призму - равен углу выхода из призмы. Следовательно:
$$
\sin (\alpha/2 + \theta/2) = n \sin(\alpha/2)
$$


Так как угол $\alpha = \frac{\pi}{3}$  и мы проводили калибровку по минимальному углу отклонения для красного цвета, получив $\theta_{red}$ можем найти $n$ для красного цвета, так как углы отличаются не сильно (3.5 градуса), то можно продолжать пользоваться приближением (угла падения и угла отражения). 
$$
n = 2 \cos (\frac{\pi}{3}-\theta/2)
$$

 Для более точного ответа, нужно брать угол падения света на призму равным $\theta_{red}/2$ и считать все преломления в системе для него. Запишем законы Снеллиуса в такой системе и распишем угол $\theta$ через другие углы системы:
$$
...
И для полечения $A$ и $B$ осталось только аппроксимировать получившиеся результаты
...
Здесь $A ≈ 1.594$, $B ≈ 9.4·10^{-3}$ (отклонение предсказывает погрешность около единицы в последней значащей цифре, но этой погрешности лучше не доверять из-за эффектов вроде недостаточной минимальности для красного цвета и так далее).

![Коэффициенты преломления, вычисленные по данным натриевой лампы](img/Na_n.png){width=60%}

Здесь $A ≈ 1.58 ± 0.01,\; B≈(13±3)·10^{-3}$, погрешность значительно выше (учитывается только случайная погрешность). Скорее всего, такой большой разброс связан со слабостью большинства линий, из-за чего их легко перепутать по цвету с друг другом. Значения лучше брать из измерений для ртутной лампы, но натриевая в целом подтверждает предыдущие результаты.
...
Мы выяснили, что совместное использование призмы и решётки - создают хороший метод измерения коэффициентов преломления по формуле Коши
