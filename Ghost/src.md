...
Табличные данные[^3] подтверждают основные линии на  589.0 и 589.6 нм, что достаточно близко к нашим "оранжевым" (опять-таки в пределах погрешности). Остальные линии, скорее всего, вызваны в первую очередь примесями, которые можно попытаться определить по линиям, однако из-за того, что мы не можем количественно оценить их интенсивность, смысла в таком определении будет немного.
...
Здесь $A ≈ 1.594$, $B ≈ 9.4·10^{3}$ нм^2^ (отклонение предсказывает погрешность около единицы в последней значащей цифре, но этой погрешности лучше не доверять из-за эффектов вроде недостаточной минимальности для красного цвета).

![Коэффициенты преломления, вычисленные по данным натриевой лампы](img/Na_n.png){width=60%}

Здесь $A ≈ 1.58 ± 0.01,\; B≈(14±4)·10^{3}$ нм^2^ , погрешность значительно выше (учитывается только случайная погрешность). Скорее всего, такой большой разброс связан со слабостью большинства линий, из-за чего их легко перепутать по цвету с друг другом. Значения лучше брать из измерений для ртутной лампы, но натриевая в целом подтверждает предыдущие результаты.
...
Мы получили, что для стекла, из которого сделана наша призма, такую зависимость можно приблизиить формулой $n(λ) = 1.594+\frac{9.4·10^{3}\mbox{нм}^2}{λ^2}$.
