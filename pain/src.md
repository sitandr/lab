...
Нетрудно заметить, что $P - P_0$ будет порядка $500 \mbox{Па}$. Что много меньше $P_0$. Значит, в показателе адиабаты множитель $P$ можно заменить на $P_0$. Тогда показатель адиабаты равен:
$$
...
Сама серия состояла из замеров частоты колебаний пробки — то есть число прерываний пробкой луча счетчика на время измерения. Как оказалось, от 2 до 10 минут на одно измерение нам вполне достаточно для того, чтобы отследить большое количество скачков пробки — за такое время число прерываний будет порядка 1 000, и случайная погрешность должна стать практически пренебрежимо малой. Наблюдения показали, что за длительное время мощность насоса меняется, что приводит к тому, что наблюдается значительное затухание колебаний, вплоть до полной остановки. Это сильно влияет на результат измерения из-за принципа работы датчика (измеряет только моменты, когда пробка достигает положения луча).
...
Видно, что для углекислого газа результаты получились слишком сильно разбросанными, чтобы можно было сделать определенный вывод. Скорее всего, это было вызвано тем, что поток не был постоянным на протяжении десяти минут, и были периоды, в которые пробки не достигала  счетчика или счетчик считывал пробку (то есть та самая проблема, описанная выше). Поэтому мы решили провести повторное измерение, в котором фиксировали время каждые 100 прерываний — это позволяет как увидеть "плохие" точки и отрезки. В целом требуется больше внимания от экспериментатора, но это позволяет  К тому же, большое количество точек позволяет оценить наклон прямой существенно точнее, чем снимая только граничные точки.
...
Коэффициент наклона аппроксимирующей прямой $5.56±0.01$ Гц (погрешность — стандартное отклонение от аппроксимации), отсюда период колебания — $T = 1/ν=2/(2N/T)=2/((5.56­±0.01))=0.360±0.001$с (совпадает с периодом, снятого в первой серии для воздуха).
...
$$
Должно было получиться примерно 5 (степень свободы двухатомного газа) — для воздуха это подтверждают табличные данные. Однако для углекислого газа при $T = 300K$ степень свободы $\sim6.9$ [^1]. У нас же получилось что-то среднее для обоих газов.
...
К сожалению, из-за технических ограничений (отсутствие штатива, плохое качество камеры, малый fps, наличие "полос" на трубке, закрывающих ее перемещение) графики не настолько качественные, чтобы можно было анализировать траекторию. 

![Пример части графика](images_src/sin_form.png){width=40%}

Видно, что это не совсем синусоида — снизу груз задерживается больше, чем сверху. Построим зависимость частоты от амплитуды:
...
В результате мы получили, что показатель адиабаты для обоих газов практически совпал ($χ≈1.32$). Это отличается от табличных данных показателей адиабаты для воздуха и углекислого газа (1.4 и 1.29 соотвественно).
...
Мы предположили, что расхождение происходит из-за того, что систему некорректно рассматривать как систему со свободными гармоническими колебаниями. И действительно, измерения показали, что частота колебаний существенно завит от амплитуды (наблюдалась разница около 40%). Вполне может быть, что это влияет на систему только на малых колебаниях, на "основной части" система должна вести себя именно так, как описывается в методике. 

Наконец, мы построили более "подробную" модель для описания изменений частоты. Тем не менее, правильных количественных предсказаний от модели добиться не удалось из-за неизвестных параметров системы, неучтенных эффектов малых колебаний и, возможно, грубых предположений о подаваемом по трубке газе. Возможно, к неверному результату привела совсем другая систематическая ошибка, которую наша модель не учитывает.  

[^1]: https://www.ohio.edu/mechanical/thermo/property_tables/gas/idealGas.html	
